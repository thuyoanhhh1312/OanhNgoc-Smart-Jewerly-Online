      ...
      include: [
        {
          ...
          attributes: ['name'],
           where: {
            name: {
              [Op.in]: ['admin', 'staff']  // chỉ lấy role admin và staff
            }
        }
      }
      ]
...
export const createUser = async (req, res) => {
  ...
  try {
    ...
    res.status(400).json({ message: "Lỗi khi tạo nhân viên", error: err.message });
  }
};
...
export const updateUser = async (req, res) => {
  ...
  try {
    const user = await db.User.findByPk(id);
    if (!user) return res.status(404).json({ message: "Nhân viên không tìm thấy" });
